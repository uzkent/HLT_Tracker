function [target]=FA_SD_Cascaded(cf,ml,hist,target,tID,bh,tm)
...
vol=((4/3)*100^3*pi)^-1;                       % Surveillance Volume
...
for n=1:tuple_cs               % Iterate through each time step
    mk=size(cf{tm-N+n}(cf{tm-N+n}<target.Sp_Fthresh),2); % # of Spectrally matching blobs k
    nk=size(cf{tm-N+n+1}(cf{tm-N+n+1}<target.Sp_Fthresh),2); % # of Spectrally matching blobs at k+1
    Sdall = find(cf{tm-N+n+1}<target.Sp_Fthresh);  % Sum of Spectral Scores
    if nk == 0                 % No Similar Obj.
        PL(n) = 1;             % Probability of Loss = 1
    else
        SumSd(n) = sum((target.Sp_Fthresh - cf{tm-N+n+1}(Sdall)));
    end
    if nk >= mk && nk > 0      % If there is more than the previous one
        PL(n) = 1/(nk+1);      % Spectral Probability of Losing T
        PM(n) = (1-PL(n));     % Spectral Probability of Matching T
    end
    if mk > nk && nk > 0         % If there is less than the previous one
        PL(n) = abs(mk-nk)/(mk); % Spectral Probability of Losing T
        PM(n) = (1-PL(n));       % Spectral Probability of Matching T
    end
end
...
for n=1:tuple_cs
    ...
    for i=1:tuple_rs
         ...
         if a(i,n)~=0                               % Non-Dummy Measurement
            ...
            if eucdis > 90                          % Distance Elimination
            ...
            Msd=target.Sp_Fthresh - cf{tm-N+n+1}(a(i,n)); % Norm. Fs
            PS=(Msd/SumSd(n));                     % Spectral likelihood
