...
leftBinIndex = ceil((angles - minAngle) / binSize);
...
% leftBinCenter = interval(leftBinIndex)'; 

% For each pixel, compute the fraction of the magnitude
% to contribute to each bin.
rightPortions = abs(angles - interval(leftBinIndex)');
leftPortions = abs(interval(rightBinIndex)' - angles);
