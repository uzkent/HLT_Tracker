...
Im_1 = matfile(sprintf('/Volumes/MY BOOK/FR_07_Circling_12pm/Full_NoTrees/Image_%d.mat',I_F+time)); 
...
if flag<2
    ...
    for j=1:size(x,1)
        ...
        target.mapped{j}(1:2) = [C_data{2}(Indx) C_data{3}(Indx)];
        ...
        [dMap,fMask,target] = SearchTargetROI(roi.sp,hist,target,model);
        ...
        actPos{time} = double([C_data{2}(Indx) C_data{3}(Indx)]); 
        if Diff(Indx) == 1 || counter == 0;
            [actPos{time}] = current2firstGT(actPos{time},model,I_F+time,I_F+1);
        else
            actPos{time} = actPos{time-1};
        end
    ...
    % The number of spectral pixels collected in forecasting step
    summation(time)=target.ds(time)+target.hs(time)+target.vs(time)+target.bs(time);
    ...
    for tID=1:size(x,1)
        ...
        target.x{tID,time}=mu; target.c{tID,time}=sig; target.ss{tID,time}=mu;   
        
        % Assignment history - Update lost tracks
        if target.as{tID}~=0 
           % Update the local target spectral pdfs
           % [hist] = updateSpdfs(blob,roi.sp,hist,target.as{tID},fMask);
           ashist(tID,time)=1; 
    ...
    target.lost = ashist(:,end); %% Update the Histogram
