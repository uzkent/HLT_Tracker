...
%   $Revision: 1.1.6.1 $  $Date: 2010/10/11 14:46:49 $
...
BW = varargin{1};
iptcheckinput(BW, {'logical' 'numeric'}, {'2d', 'real', 'nonsparse'}, ...
    mfilename, 'BW', 1);
...
method = iptcheckstrs(method,{'union','objects'},mfilename,'METHOD',2);
...
if ~is_valid_matrix
    eid = sprintf('Images:%s:invalidConnectivity',mfilename);
    error(eid,'%s%s%s%s','Invalid connectivity argument.  ',...
        upper(mfilename),' expects its 3rd argument, CONN, to be ',...
        '4, 8, or a valid 3x3 symmetric connectivity matrix.');
end