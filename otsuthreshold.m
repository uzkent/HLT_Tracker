function [Mask,target] = otsuthreshold(Dist,target,ml,time)
% -------------------------------------------------------------------------
%   Explanation : 
...
if time == 2
    ...
    target.Sp_Fthresh = target.Sp_Thresh{1};
else 
    if target.as{1} ~= 0
        target.Sp_Fthresh = (ml.alpha*target.Sp_Fthresh+(1-ml.alpha)*target.Sp_Thresh{2});
