function [Mask,target] = otsuthreshold_SeparateBands2(Dist,target,ml,time,Index)
% -------------------------------------------------------------------------
%   Explanation : 
...
if time == 2
    target.Sp_Thresh{1} = double(multithresh(Temp,ml.level3+5));
    ...
    if target.as{1} ~= 0
        ...
        target.Sp_Thresh{2} = double(multithresh(Temp,ml.level3));
